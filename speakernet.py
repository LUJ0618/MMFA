...
from load_data import test_dataset_loader
import pickle
...
class WrappedModel(nn.Module):
    ...
    def forward(self, x, label=None, l2_reg_dict=None):
        return self.module(x, label, l2_reg_dict)

    # def update_epoch(self):
    #         """ 🔥 내부 모델(SpeakerNet)의 update_epoch() 호출 """
    #         if hasattr(self.module, "update_epoch"):
    #             self.module.update_epoch()
    #         else:
    #             print("⚠️ [WrappedModel] No update_epoch() method found in wrapped model")
...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, **kwargs):
        ...
        self.mask_ratio = kwargs.get("mask_ratio", 0.0)
        print(f"🔹 [SpeakerNet] Using mask_ratio from YAML: {self.mask_ratio}")
    ...
    # def update_epoch(self):
    #     if hasattr(self.__S__, "update_epoch"):
    #         print(f"🔹 [WrappedModel] Forwarding update_epoch() to {self.module.__class__.__name__}")
    #         self.__S__.update_epoch()
    #     else:
    #         print("⚠️ [WrappedModel] No update_epoch() method found in wrapped model")
...
class ModelTrainer(object):
    ...
    def evaluateAttn(self, test_list, test_path, nDataLoaderThread, print_interval=10, num_eval=10, **kwargs):
        """
        기존 평가 코드 + 어텐션 맵을 시각화하는 코드
        """
        self.__model__.eval()
        ...
        ## Get a list of unique file names (상위 5개만 선택)
        files = sum([x.strip().split()[-2:] for x in lines[2050:2051]], [])
        setfiles = list(set(files))
        setfiles.sort()
        print('After sorting (5 files selected)')
        ...
        os.makedirs("./attention_maps_mask70", exist_ok=True)  # 🔥 저장할 폴더 생성
        ...
        for idx, data in enumerate(test_loader):
            ...
            inp1 = data[0][0].cuda()
            file_name = os.path.basename(data[2][0]).split(".")[0]  # 🔥 확장자 제거한 파일 이름
            print(data[2][0])

            # 🔹 모델의 attention map 저장 함수 호출
            save_path = f"./attention_maps/attn_map_{file_name}.png"
            ref_feat            = self.__model__([inp1, "test"]).cuda()
