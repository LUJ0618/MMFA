...
parser.add_argument('--config',         type=str,   default="./yaml/baseline.yaml",   help='Config YAML file');
...
parser.add_argument('--pretrained_model_path',    type=str,    default="baseline/pretrained/WavLM-Base+.pt",  help='Absolute path to the pre-trained model');
...
## Training and test data
parser.add_argument('--train_list', type=str, default="../dataset/speaker_verification/voxceleb2_train.txt", help='Train list');
parser.add_argument('--test_list', type=str, default="../dataset/speaker_verification/voxceleb1_O_clean.txt", help='Evaluation list');

parser.add_argument('--train_path', type=str, default="../dataset/speaker_verification/voxceleb2/", help='Absolute path to the train set');
parser.add_argument('--test_path', type=str, default="../dataset/speaker_verification/voxceleb1_test/", help='Absolute path to the test set');
parser.add_argument('--musan_path', type=str, default="../dataset/speaker_verification/musan/", help='Absolute path to the test set');
parser.add_argument('--rir_path', type=str, default="../dataset/speaker_verification/rirs_noises/RIRS_NOISES/simulated_rirs/", help='Absolute path to the test set');
...
parser.add_argument('--mask_ratio', type=float, default=0.0, help='Frame masking ratio for ASPM (0.0 ~ 1.0)')

## For test only
parser.add_argument('--eval',           dest='eval', action='store_true', help='Eval only')

## Distributed and mixed precision training
parser.add_argument('--gpu-id', type=int, default=0, help='avaliable gpu id (0 ~ 5)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ## for testing specific epoch model 
    modelfiles = os.path.join(args.model_save_path, "model000000015.model")
    # modelfiles = os.path.join("../dataset/speaker_verification/exp/model/", "model000000020.model")
    print(modelfiles)
    ...
    if(args.initial_model != ""):
        ...
        print("Model {} loaded from previous state!".format(modelfiles));
        trainer.loadParameters(modelfiles);
        it = int(os.path.splitext(os.path.basename(modelfiles))[0][5:]) + 1
    ...
    if args.eval == True:
        ...
        trainer.evaluateAttn(**vars(args))
...
def main():
    ...
    # else:
    main_worker(args.gpu_id, None, args)
